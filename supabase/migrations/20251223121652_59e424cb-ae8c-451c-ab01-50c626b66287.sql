-- SECURITY FIX: Garantir apenas 1 license ativa por usuário
-- Atualizar complete_order_atomic para cancelar licenses anteriores
...
BEGIN
  ...
  IF _product_type = 'subscription' THEN
    ...
    -- ==========================================
    -- CRITICAL: Cancel ALL previous active subscriptions for this user
    ...
    -- Cancel all active licenses for this user
    UPDATE licenses
    SET status = 'cancelled',
        updated_at = NOW()
    WHERE user_id = _user_id
