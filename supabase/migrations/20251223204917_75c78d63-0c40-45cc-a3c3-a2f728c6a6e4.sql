-- ==========================================
-- 1. TRIGGER: Sincronizar license.status com end_date automaticamente
-- Força status='expired' quando end_date < NOW()
-- ==========================================

-- Função que verifica e atualiza o status da licença
CREATE OR REPLACE FUNCTION public.auto_expire_license_on_access()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
BEGIN
  -- Se a licença está ativa mas já passou da data de expiração, forçar expired
  IF NEW.status = 'active' AND NEW.end_date < NOW() THEN
    NEW.status := 'expired';
    NEW.updated_at := NOW();
    RAISE NOTICE 'Auto-expired license % for user %', NEW.id, NEW.user_id;
  END IF;
...
-- Trigger que roda ANTES de qualquer SELECT retornar dados (via UPDATE)
-- e também antes de INSERT/UPDATE para garantir consistência
CREATE OR REPLACE TRIGGER trigger_auto_expire_license
  BEFORE INSERT OR UPDATE ON public.licenses
  FOR EACH ROW
  EXECUTE FUNCTION public.auto_expire_license_on_access();

-- ==========================================
-- 2. ATUALIZAR complete_order_atomic com validações de backend
-- - Validar upgrade_credit_amount no servidor
-- - Validar max_subscriptions_per_user  
-- - Validar quantidade permitida
-- ==========================================
...
BEGIN
  ...
  IF _product_type = 'subscription' THEN
    ...
    -- ==========================================
    -- NEW: BACKEND VALIDATION - max_subscriptions_per_user
    -- ==========================================
    ...
    IF _max_subs_per_user IS NOT NULL THEN
      ...
      IF _current_sub_count >= _max_subs_per_user THEN
        RETURN json_build_object(
          ...
          'current_count', _current_sub_count,
          'max_allowed', _max_subs_per_user
        );
      END IF;
    END IF;
    
    -- ==========================================
    -- NEW: BACKEND VALIDATION - upgrade_credit_amount
    -- Recalcula o crédito esperado e compara com o que está na order
    -- Tolerância de R$0.50 para evitar problemas de arredondamento
    -- ==========================================
    IF _upgrade_from_subscription_id IS NOT NULL AND _actual_credit_on_order > 0 THEN
      -- Buscar a subscription antiga e recalcular o crédito
      SELECT 
      ...
      IF _current_plan_price IS NOT NULL AND _plan_period > 0 AND _days_remaining > 0 THEN
        ...
        -- Se o crédito na order diverge mais de R$0.50 do esperado, rejeitar
        IF ABS(_actual_credit_on_order - _expected_credit) > 0.50 THEN
          RETURN json_build_object(
            'success', false, 
            'error', 'Crédito de upgrade inválido - possível manipulação',
            'expected_credit', _expected_credit,
            'received_credit', _actual_credit_on_order,
            'difference', ABS(_actual_credit_on_order - _expected_credit)
          );
        END IF;
      END IF;
    END IF;
    ...
    RETURN json_build_object(
      ...
      'credit_validated', _actual_credit_on_order > 0
    );
  END IF;
...
-- ==========================================
-- 3. Expirar licenças que já deveriam ter expirado
-- (limpeza inicial para consistência)
-- ==========================================
UPDATE licenses
