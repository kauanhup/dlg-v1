...
// Sales Chart Component using real data
const SalesChart = ({ orders }: { orders: { product_type: string; amount: number; created_at: string; status: string }[] }) => {
  ...
  const monthlyData = orders
    ...
    .reduce((acc, order) => {
      ...
      if (!acc[monthKey]) {
        acc[monthKey] = { brasileiras: 0, estrangeiras: 0, receita: 0 };
      }
      if (order.product_type === 'brasileiras') {
        acc[monthKey].brasileiras += 1;
      } else {
        acc[monthKey].estrangeiras += 1;
      }
      acc[monthKey].receita += Number(order.amount);
      return acc;
    }, {} as Record<string, { brasileiras: number; estrangeiras: number; receita: number }>);

  const data = Object.entries(monthlyData).slice(-6).map(([mes, values]) => ({
    mes,
    ...values
  }));
