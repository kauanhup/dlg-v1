            ...
            <Accordion type="single" collapsible className="space-y-2">
              {FAQ_ITEMS.map((faq, index) => (
                <motion.div 
                  ...
                  initial={{ opacity: 0, y: 15 }}
                  animate={isInView ? { opacity: 1, y: 0 } : {}}
                  transition={{ delay: 0.2 + index * 0.05, duration: 0.3, ease }}
                >
                  <AccordionItem 
                    value={`item-${index}`}
                    className="border border-border rounded-lg px-3 sm:px-4 bg-background data-[state=open]:border-primary/30 transition-all duration-200 hover:border-border/80"
                  >
